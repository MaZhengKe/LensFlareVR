...
namespace UnityEngine.Rendering
{
    ...
    /// </summary>
    public sealed class LensFlareCommonMK
    {
        private static LensFlareCommonMK m_Instance = null;
        private static readonly object m_Padlock = new object();

        private static System.Collections.Generic.List<LensFlareComponentMK> m_Data =
            new System.Collections.Generic.List<LensFlareComponentMK>();
        ...
        static public void Initialize()
        {
            if (occlusionRT == null && mergeNeeded > 0)
                occlusionRT = RTHandles.Alloc(width: maxLensFlareWithOcclusion,
                    height: maxLensFlareWithOcclusionTemporalSample + 1 * mergeNeeded,
                    colorFormat: Experimental.Rendering.GraphicsFormat.R16_SFloat, enableRandomWrite: true,
        ...
        /// </summary>
        public static LensFlareCommonMK Instance
        {
            get
            {
                if (m_Instance == null)
                {
                    lock (m_Padlock)
                    {
                        if (m_Instance == null)
                        {
                            m_Instance = new LensFlareCommonMK();
                        }
                    }
                }
        ...
        private System.Collections.Generic.List<LensFlareComponentMK> Data
        {
            get { return LensFlareCommonMK.m_Data; }
        }
        ...
        public System.Collections.Generic.List<LensFlareComponentMK> GetData()
        {
            return Data;
        }
        ...
        public void AddData(LensFlareComponentMK newData)
        {
            Debug.Assert(Instance == this, "LensFlareCommonMK can have only one instance");
        ...
        /// <returns>Attenuation Factor</returns>
        static public float ShapeAttenuationSpotConeLight(Vector3 forward, Vector3 wo, float spotAngle,
        ...
        /// <returns>Attenuation Factor</returns>
        static public float ShapeAttenuationAreaTubeLight(Vector3 lightPositionWS, Vector3 lightSide, float lightWidth,
            Camera cam)
        {
        ...
        static public Vector4 GetFlareData0(Vector2 screenPos, Vector2 translationScale, Vector2 rayOff0,
            Vector2 vLocalScreenRatio, float angleDeg, float position, float angularOffset, Vector2 positionOffset,
            bool autoRotate)
        {
            ...
            return new Vector4(localCos0, localSin0, positionOffset.x + rayOff0.x * translationScale.x,
        ...
        static Vector3 WorldToViewport(Camera.MonoOrStereoscopicEye eye, Camera camera, bool isLocalLight,
            bool isCameraRelative, Matrix4x4 viewProjMatrix, Vector3 positionWS)
        {
            if (isLocalLight)
            {
                if (eye == Camera.MonoOrStereoscopicEye.Mono)
                    return WorldToViewportLocal(isCameraRelative, viewProjMatrix, camera.transform.position,
                        positionWS);
                else
                {
                    var eyePos = GetEyePosition((Side)eye);
                    return WorldToViewportLocal(isCameraRelative, viewProjMatrix, eyePos, positionWS);
                }
            }
            else
            {
                return WorldToViewportDistance(eye, camera, positionWS);
            }
        }
        ...
        static Vector3 WorldToViewportLocal(bool isCameraRelative, Matrix4x4 viewProjMatrix, Vector3 cameraPosWS,
            Vector3 positionWS)
        {
        ...
        static Vector3 WorldToViewportDistance(Camera.MonoOrStereoscopicEye eye, Camera cam, Vector3 positionWS)
        {
            if (eye == Camera.MonoOrStereoscopicEye.Mono)
            {
            ...
            {
                Vector4 camPos = cam.GetStereoViewMatrix((Camera.StereoscopicEye)eye) * positionWS;
                Vector4 viewportPos4 = cam.GetStereoProjectionMatrix((Camera.StereoscopicEye)eye) * camPos;
                Vector3 viewportPos = new Vector3(viewportPos4.x, viewportPos4.y, 0f);
                viewportPos /= viewportPos4.w;
                viewportPos.x = viewportPos.x * 0.5f + 0.5f;
                viewportPos.y = viewportPos.y * 0.5f + 0.5f;
        ...
        static public void ComputeOcclusion(Camera.MonoOrStereoscopicEye eye, Material lensFlareShader,
            LensFlareCommonMK lensFlares, Camera cam,
            ...
            bool taaEnabled,
            int _FlareOcclusionTex, int _FlareOcclusionIndex, int _FlareTex, int _FlareColorValue, int _FlareData0,
            int _FlareData1, int _FlareData2, int _FlareData3, int _FlareData4)
        {
            ...
            foreach (LensFlareComponentMK comp in lensFlares.GetData())
            {
                if (comp == null)
                    continue;
                ...
                {
                    viewportPos = DoPaniniProjection(viewportPos, actualWidth, actualHeight, cam.fieldOfView,
                        paniniCropToFit, paniniDistance);
                }
                ...
                float distanceAttenuation = !isDirLight && comp.distanceAttenuationCurve.length > 0
                    ? comp.distanceAttenuationCurve.Evaluate(coefDistSample)
                    : 1.0f;
                float scaleByDistance = !isDirLight && comp.scaleByDistanceCurve.length >= 1
                    ? comp.scaleByDistanceCurve.Evaluate(coefScaleSample)
                    : 1.0f;

                Vector3 dir = (cam.transform.position - comp.transform.position).normalized;
                Vector3 screenPosZ = WorldToViewport(eye, cam, !isDirLight, isCameraRelative, viewProjMatrix,
                ...
                Vector2 occlusionRadiusEdgeScreenPos1 = (Vector2)WorldToViewport(eye, cam, !isDirLight,
                    isCameraRelative, viewProjMatrix, positionWS + cam.transform.up * adjustedOcclusionRadius);
                ...
                cmd.SetGlobalVector(_FlareOcclusionIndex,
                    new Vector4(((float)(occlusionIndex)) * dx + halfx, halfy, 0, frameIdx + 1));
                ...
                cmd.SetGlobalVector(_FlareData3,
                    new Vector4(comp.allowOffScreen ? 1.0f : -1.0f, usedGradientPosition,
                        Mathf.Exp(Mathf.Lerp(0.0f, 4.0f, 1.0f)), 1.0f / 3.0f));

                Vector2 rayOff = GetLensFlareRayOffset(screenPos, position, globalCos0, globalSin0);
                Vector4 flareData0 = GetFlareData0(screenPos, Vector2.one, rayOff, vScreenRatio, 0.0f, position, 0.0f,
                    Vector2.zero, false);
                ...
                cmd.SetViewport(new Rect()
                    { x = occlusionIndex, y = (frameIdx + 1 * mergeNeeded) * taaValue, width = 1, height = 1 });


                cmd.DrawProcedural(Matrix4x4.identity, lensFlareShader, 4, MeshTopology.Quads, 4, 1, s_PropertyBlock);
        ...
        static public void DoLensFlareDataDrivenCommon(Material lensFlareShader, LensFlareCommonMK lensFlares,
            Camera cam, float actualWidth, float actualHeight,
            bool usePanini, float paniniDistance, float paniniCropToFit,
            bool isCameraRelative,
            Vector3 cameraPositionWS,
            Matrix4x4 viewProjMatrix,
            Matrix4x4 LeftViewProjMatrix,
            Matrix4x4 RightViewProjMatrix,
            Rendering.CommandBuffer cmd,
            ...
            bool debugView)
        {
            ...
            foreach (LensFlareComponentMK comp in lensFlares.GetData())
            {
                ...
                viewportPos = WorldToViewport(Camera.MonoOrStereoscopicEye.Mono, cam, !isDirLight, isCameraRelative, viewProjMatrix, positionWS);
                ...
                Vector3 dir = (cam.transform.position - comp.transform.position).normalized;
                Vector3 screenPosZ = WorldToViewport(Camera.MonoOrStereoscopicEye.Mono, cam, !isDirLight, isCameraRelative, viewProjMatrix,
                    positionWS + dir * comp.occlusionOffset);

                float adjustedOcclusionRadius =
                    isDirLight ? comp.celestialProjectedOcclusionRadius(cam) : comp.occlusionRadius;
                Vector2 occlusionRadiusEdgeScreenPos0 = (Vector2)viewportPos;
                Vector2 occlusionRadiusEdgeScreenPos1 = (Vector2)WorldToViewport(Camera.MonoOrStereoscopicEye.Mono, cam, !isDirLight,
                    isCameraRelative, viewProjMatrix, positionWS + cam.transform.up * adjustedOcclusionRadius);
                float occlusionRadius = (occlusionRadiusEdgeScreenPos1 - occlusionRadiusEdgeScreenPos0).magnitude;
                cmd.SetGlobalVector(_FlareData1,
                    new Vector4(occlusionRadius, comp.sampleCount, screenPosZ.z, actualHeight / actualWidth));
                ...
                cmd.SetGlobalVector(_FlareOcclusionIndex,
                    new Vector4(
                        (float)occlusionIndex / (float)LensFlareCommonMK.maxLensFlareWithOcclusion +
                        0.5f / (float)LensFlareCommonMK.maxLensFlareWithOcclusion, 0.5f, 0, 0));
                ...
                foreach (LensFlareDataElementSRP element in data.elements)
                {
                    ...
                    if (light != null && element.modulateByLightColor)
                    {
                        if (light.useColorTemperature)
                            colorModulation *=
                                light.color * Mathf.CorrelatedColorTemperatureToRGB(light.colorTemperature);
                        else
                    ...
                    float currentIntensity = comp.intensity * element.localIntensity * radialsScaleRadius *
                                             distanceAttenuation;
                    ...
                    float angularOffset = SystemInfo.graphicsUVStartsAtTop
                        ? element.angularOffset
                        : -element.angularOffset;
                    float globalCos0 = Mathf.Cos(-angularOffset * Mathf.Deg2Rad);
                    float globalSin0 = Mathf.Sin(-angularOffset * Mathf.Deg2Rad);
                    ...
                    Vector2 ComputeLocalSize(Vector2 rayOff, Vector2 rayOff0, Vector2 curSize,
                        AnimationCurve distortionCurve)
                    {
                        ...
                        if (!element.distortionRelativeToCenter)
                        {
                            ...
                            localRadius =
                                Mathf.Clamp01(Mathf.Max(Mathf.Abs(localRadPos.x),
                                    Mathf.Abs(localRadPos.y))); // l1 norm (instead of l2 norm)
                        }
                        else
                        {
                            localRadPos = screenPos +
                        ...
                        return new Vector2(
                            Mathf.Lerp(curSize.x, element.targetSizeDistortion.x * combinedScale / usedAspectRatio,
                                localLerpValue),
                    ...
                    cmd.SetGlobalVector(_FlareData3,
                        new Vector4(
                            ...
                            //eye == Camera.MonoOrStereoscopicEye.Mono?2:(eye ==Camera.MonoOrStereoscopicEye.Left?1:0)
                            1.0f / (float)element.sideCount
                        ));
                    ...
                    if (!element.allowMultipleElement || element.count == 1)
                    {
                        ...
                        Vector4 flareData0 = GetFlareData0(screenPos, element.translationScale, rayOff, vScreenRatio,
                            rotation, position, angularOffset, element.positionOffset, element.autoRotate);

                        cmd.SetGlobalVector(_FlareData0, flareData0);
                        cmd.SetGlobalVector(_FlareData2,
                        ...
                        cmd.DrawProcedural(Matrix4x4.identity, lensFlareShader, materialPass, MeshTopology.Quads, 4, 1,
                            s_PropertyBlock);
                    }
                    else
                    {
                        ...
                        switch (element.distribution)
                        {
                            case SRPLensFlareDistribution.Uniform:
                            {
                                ...
                                for (int elemIdx = 0; elemIdx < element.count; ++elemIdx)
                                {
                                    ...
                                    {
                                        Vector2 rayOff0 =
                                            GetLensFlareRayOffset(screenPos, 0.0f, globalCos0, globalSin0);
                                        localSize = ComputeLocalSize(rayOff, rayOff0, localSize,
                                            element.distortionCurve);
                                    }

                                    float timeScale = element.count >= 2
                                        ? ((float)elemIdx) / ((float)(element.count - 1))
                                        : 0.5f;

                                    Color col = element.colorGradient.Evaluate(timeScale);

                                    Vector4 flareData0 = GetFlareData0(screenPos, element.translationScale, rayOff,
                                        vScreenRatio, rotation + uniformAngle, position, angularOffset,
                                        element.positionOffset, element.autoRotate);
                                    cmd.SetGlobalVector(_FlareData0, flareData0);
                                    cmd.SetGlobalVector(_FlareData2,
                                        new Vector4(screenPos.x, screenPos.y, localSize.x, localSize.y));
                                    ...
                                    cmd.DrawProcedural(Matrix4x4.identity, lensFlareShader, materialPass,
                                        MeshTopology.Quads, 4, 1, s_PropertyBlock);
                            ...
                            case SRPLensFlareDistribution.Random:
                            {
                                ...
                                for (int elemIdx = 0; elemIdx < element.count; ++elemIdx)
                                {
                                    ...
                                    float localRotation = rotation +
                                                          RandomRange(-Mathf.PI, Mathf.PI) * element.rotationVariation;

                                    if (localIntensity > 0.0f)
                                    {
                                        Vector4 flareData0 = GetFlareData0(screenPos, element.translationScale, rayOff,
                                            vScreenRatio, localRotation, position, angularOffset, localPositionOffset,
                                            element.autoRotate);
                                        cmd.SetGlobalVector(_FlareData0, flareData0);
                                        cmd.SetGlobalVector(_FlareData2,
                                            new Vector4(screenPos.x, screenPos.y, localSize.x, localSize.y));
                            ...
                            case SRPLensFlareDistribution.Curve:
                            {
                                for (int elemIdx = 0; elemIdx < element.count; ++elemIdx)
                                {
                                    ...
                                    float positionSpacing = element.positionCurve.length > 0
                                        ? element.positionCurve.Evaluate(timeScale)
                                        : 1.0f;
                                    ...
                                    float sizeCurveValue = element.scaleCurve.length > 0
                                        ? element.scaleCurve.Evaluate(timeScale)
                                        : 1.0f;
                                    localSize *= sizeCurveValue;

                                    float angleFromCurve = element.uniformAngleCurve.Evaluate(timeScale) *
                                                           (180.0f - (180.0f / (float)element.count));

                                    Vector4 flareData0 = GetFlareData0(screenPos, element.translationScale, rayOff,
                                        vScreenRatio, rotation + angleFromCurve, localPos, angularOffset,
                                        element.positionOffset, element.autoRotate);
                                    cmd.SetGlobalVector(_FlareData0, flareData0);
                                    cmd.SetGlobalVector(_FlareData2,
                                        new Vector4(screenPos.x, screenPos.y, localSize.x, localSize.y));
        ...
        public void RemoveData(LensFlareComponentMK data)
        {
            Debug.Assert(Instance == this, "LensFlareCommonMK can have only one instance");
        ...
        static Vector2 DoPaniniProjection(Vector2 screenPos, float actualWidth, float actualHeight, float fieldOfView,
            float paniniProjectionCropToFit, float paniniProjectionDistance)
        {
